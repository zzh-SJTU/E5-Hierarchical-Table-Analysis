...
output_file_name = args.option + "_ReAct_prediction-gpt-4-32k.jsonl"
...
Final Answer: the final answer to the original input question (should be a list of comma separated values, eg: `foo, bar, baz`. If the answer only contains one element, the list should only contains one element.)
    ...
    if "label" in data.keys():
        ...
        if len(data["label"]) == len(data[args.option + ' prediction']):
            ...
            for i in range(len(data["label"])):
                if_match, pred, label = eval_ex_match(str(data["label"][i]), data[args.option + ' prediction'][i])
                if if_match == False:
                    flag = False
            if flag == True:
                corret += 1
