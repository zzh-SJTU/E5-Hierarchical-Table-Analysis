...
def find_non_empty_cell_positions(top_header_rows_num, left_header_columns_num, html_table, initial_prompt, max_tokens, original_html_table, strategy='spread'):  
    ...
    while True:  
        ...
        for row, col in current_positions:  
            if strategy == 'spread':  
                ...
                for r, c in spread_order:  
                    if 0 <= r < total_rows and 0 <= c < total_columns and matrix[r][c] is None:  
                        if update_matrix_and_tokens(r, c):  
                            updated = True  
    ...
    # Update html_table with values from original_html_table based on matrix  
    for row_index, row in enumerate(table.find_all('tr')[top_header_rows_num:]):  
        for col_index, cell in enumerate(row.find_all('td')[left_header_columns_num:]): 
            if row_index < len(matrix):
                if col_index < len(matrix[row_index]):
...
output_file_name = args.option + "_prediction-gpt-4-32k_spread_" + str(args.token_limit) + "-1.jsonl"
