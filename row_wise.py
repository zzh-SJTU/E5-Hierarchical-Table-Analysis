...
def find_non_empty_cell_positions(top_header_rows_num, left_header_columns_num, html_table, initial_prompt, max_tokens, original_html_table):  
    ...
    total_columns = max(  
        len(row.find_all('td')) - left_header_columns_num + sum(int(td.get('colspan', 1)) - 1 for td in row.find_all('td')) for row in table.find_all('tr')[top_header_rows_num:]  
    )  
    ...
    if len(positions) == 0:
        import random  
        positions = [(random.randint(0, total_rows - 1), random.randint(0, total_columns - 1))]
    ...
    while True:  
        ...
        for row, col in current_positions:  
            if row - 1 >= 0:  
                for col_index in range(total_columns):  
                    if matrix[row - 1][col_index] is None:  
                        if update_matrix_and_tokens(row - 1, col_index):  
                            updated = True  
            if row + 1 < total_rows:  
                for col_index in range(total_columns):  
                    if matrix[row + 1][col_index] is None:  
...
output_file_name = args.option + "_prediction-turbo_row_" + str(args.token_limit) + "-4.jsonl"
...
for i  in reversed(range(len(test_data_list))):
